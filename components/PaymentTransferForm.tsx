...
const formSchema = z.object({
  ...
  }, "Transfer limit exceeded (Max: ₹1,000,000)"),
  ...
  const submit = async (data: z.infer<typeof formSchema>) => {
    ...
    try {
      ...
      try {
        ...
        throw new Error("Invalid Sharable ID format (must be a valid connected bank ID).");
      }

      const selectedSender = accounts.find(
        (account) => account.appwriteItemId === data.senderBank
      );
      ...
      const response = await fetch(`${API_BASE}/api/transfer`, {
        ...
        credentials: "include",
        body: JSON.stringify({
          senderAccountId: selectedSender.appwriteItemId,
